...
ISR(ADC_vect) {
	...
	//LCD_Display('');
	//CD_Display(((voltage/1000)%10) + '0'); // hundreds
	//LCD_Display(((voltage/100)%10) + '0'); // hundreds
	LCD_Display(((voltage/1000)%10) + '0'); // tens
	LCD_Display('.');
	LCD_Display((voltage/100) % 10 + '0'); // ones
	LCD_Display((voltage/10)%10 + '0');
	//LCD_Display((voltage % 100) + '0'); // ones
	}
	oldVoltage = voltage;
...
int main(void)
{
	...
	DDRB = (1<<PORTB0)|(1<<PORTB2)|(0<<PORTB3)|(0<<PORTB4)|(1<<PORTB5);
	//PORTB |= (1<<PORTD4);
    //CONFIGURE IO
	...
	ADCSRA = (1 << ADEN) | (1 << ADPS2) | (1 << ADPS1) | (1 << ADPS0); // start first conversion
	...
	SPCR = (0<<SPIE)|(1<<SPE)|(1<<MSTR)|(1<<CPOL)|(0<<CPHA)|(0<<SPR1)|(1<<SPR0);
	SPSR = (1<<SPI2X);
